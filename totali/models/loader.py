...
Loader responsibilities (AGENTIC.md totali/models/AGENTIC.md):

- File existence and readability
- Optional SHA-256 hash check against a `models/MANIFEST.json` entry
- Deterministic session config flags available to callers
- Refusal to download; operators provision out-of-band

The loader is intentionally thin — heavy lifting happens inside `onnxruntime`
which is stubbed under pytest. The public surface is:

    load_onnx(path, *, device="cpu", deterministic=True, expected_sha256=None)
...
def load_manifest(manifest_path: Optional[Path] = None) -> dict[str, dict[str, Any]]:
    ...
    enforcement). A malformed manifest raises `ManifestError`.
    """
    path = manifest_path or (Path("models") / "MANIFEST.json")
    if not path.exists():
        return {}
    try:
...
def load_onnx(
    ...
    *,
    ...
    Returns an `onnxruntime.InferenceSession`. Raises:
    - `ModelNotFoundError` if the file is absent.
    - `ModelHashMismatch` if SHA-256 disagrees with `expected_sha256` or the
      manifest entry for this filename.
    - Whatever `onnxruntime.InferenceSession(...)` raises (ImportError when
      onnxruntime is unavailable — callers should catch and fall back).
    """
