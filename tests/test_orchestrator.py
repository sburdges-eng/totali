...
class TestSinglePhaseExecution:
    def test_invalid_phase_raises_value_error(self, orchestrator, tmp_path):
        fake_las = tmp_path / "input.las"
        fake_las.write_bytes(b"\x00" * 100)
        with pytest.raises(ValueError, match=r"Unknown phase .* Must be one of:"):
            orchestrator.run(str(fake_las), phase="invalid_phase")
