...
# Stub pyproj (only when not installed)
_ensure_stub("pyproj", {
    "CRS": _FakeCRS,
...
# absent so shield's try/except falls back to _write_dxf_manual. Tests that
# exercise DXF writing use explicit mocks.
_ensure_stub("ezdxf")

# Stub onnxruntime so import succeeds; InferenceSession raises ImportError
# so classifier._load_model() catches it and uses rule-based fallback.
def _fake_inference_session(*_args, **_kwargs):
    raise ImportError("onnxruntime not available")
_ensure_stub("onnxruntime", {"InferenceSession": _fake_inference_session})
