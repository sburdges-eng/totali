...
class TestComputeSha256:
    ...
    def test_deterministic_repeated_call(self, tmp_path):
        ...
        f.write_bytes(b"some bytes to hash" * 100)
...
class TestLoadManifest:
    ...
    def test_entry_not_object_raises(self, tmp_path):
        path = tmp_path / "m.json"
        path.write_text(json.dumps({"seg.onnx": "not-an-object"}))
        with pytest.raises(ManifestError):
            load_manifest(path)
...
class TestLoadOnnxGuards:
    ...
    def test_matching_hash_proceeds_to_onnx_layer(self, tmp_path):
        """Hash match must clear the guards; the stub onnxruntime module is
        empty, so loader raises ImportError — proving we reached the ort layer.
        """
        m = tmp_path / "m.onnx"
    ...
    def test_matching_hash_with_empty_manifest(self, tmp_path):
        """With an explicit empty manifest and no expected_sha256, guards pass."""
        m = tmp_path / "m.onnx"
        m.write_bytes(b"payload")
        with pytest.raises(ImportError):
            load_onnx(m, manifest={})
