...
class TestContourGeneration:
    ...
    def test_zero_interval_returns_empty_contours(self, extractor, ground_points):
        """Invalid config interval_ft <= 0 or index_interval_ft <= 0 must not crash."""
        vertices, faces, _ = extractor._build_dtm(ground_points)
        extractor.cnt_cfg["interval_ft"] = 0.0
        minor, index, metrics = extractor._generate_contours(vertices, faces)
        assert minor == []
        assert index == []
        assert metrics["minor_count"] == 0
        assert metrics["index_count"] == 0

    def test_zero_index_interval_does_not_raise(self, extractor, ground_points):
        """index_interval_ft <= 0 must not raise ZeroDivisionError."""
        vertices, faces, _ = extractor._build_dtm(ground_points)
        extractor.cnt_cfg["index_interval_ft"] = 0.0
        minor, index, metrics = extractor._generate_contours(vertices, faces)
        assert metrics["index_count"] == 0
        assert "elevation_range" in metrics
